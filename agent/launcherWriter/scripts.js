...
function createFirewallRuleFunctions() {
  return [
    ...
    "}",
    "function Unblock-RuntimeExecutable([string]$programPath) {",
    "  if (-not $programPath -or -not (Test-Path $programPath)) { return }",
    "  try { Unblock-File -LiteralPath $programPath -ErrorAction SilentlyContinue } catch {}",
    "  try { Remove-Item -LiteralPath ($programPath + ':Zone.Identifier') -Force -ErrorAction SilentlyContinue } catch {}",
    "}",
    "function Set-RuntimeExecutablePermissions([string]$runtimeDir) {",
    ...
    "      & $icaclsPath $runtimeDir /inheritance:e /grant '*S-1-5-18:(OI)(CI)F' '*S-1-5-32-544:(OI)(CI)F' '*S-1-5-32-545:(OI)(CI)RX' /T /C *> $null",
    "    }",
    "  } catch {}",
    "}",
    ...
    "    $icaclsPath = Join-Path $env:SystemRoot 'System32\\icacls.exe'",
    "    if (Test-Path $icaclsPath) {",
    "      & $icaclsPath $installDir /inheritance:e /grant '*S-1-5-18:(OI)(CI)F' '*S-1-5-32-544:(OI)(CI)F' '*S-1-5-32-545:(OI)(CI)RX' /T /C *> $null",
    "    }",
    "  } catch {}",
    "}",
    ...
    "    Unblock-RuntimeExecutable $runtimeCloudflaredPath",
    "  }",
...
function createPowerShellScripts() {
  const registerStartupPs1 = [
    ...
    `$watchdogTaskName = '${WATCHDOG_TASK_NAME.replace(/'/g, "''")}'`,
    `$watchdogTaskDescription = '${WATCHDOG_TASK_DESCRIPTION.replace(/'/g, "''")}'`,
    ...
    "    $icaclsPath = Join-Path $env:SystemRoot 'System32\\icacls.exe'",
    '    if ((Test-Path $taskPath) -and (Test-Path $icaclsPath)) {',
    "      & $icaclsPath $taskPath /grant '*S-1-5-32-545:RX' /C *> $null",
    '    }',
    '  } catch {',
    ...
    '}',
    'function Register-WatchdogTask([string]$name, [string]$description, [string]$scriptPath) {',
    '  $argument = \'-NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File "\' + $scriptPath + \'"\'',
    '  $action = New-ScheduledTaskAction -Execute $powerShellPath -Argument $argument',
    '  $repeatStart = (Get-Date).Date.AddMinutes(1)',
    '  $periodicTrigger = New-ScheduledTaskTrigger -Once -At $repeatStart -RepetitionInterval (New-TimeSpan -Minutes 1) -RepetitionDuration (New-TimeSpan -Days 3650)',
    ...
    '  Register-ScheduledTask @taskParams | Out-Null',
    '  Grant-TaskRunAccess $name',
    '  Write-StartupLog ("Registered watchdog task " + $name + " -> " + $scriptPath)',
    '}',
    'Register-ServiceTask $startupTaskName $startupTaskDescription (Join-Path $installDir "start-supervisor.ps1") $true',
    'Register-WatchdogTask $watchdogTaskName $watchdogTaskDescription (Join-Path $installDir "start-supervisor.ps1")',
  ...
  const adminStartServicePs1 = [
    ...
    'try { Enable-ScheduledTask -TaskName $watchdogTaskName -ErrorAction SilentlyContinue | Out-Null } catch {}',
  ...
  const postInstallPs1 = [
    ...
    '  & (Join-Path $installDir "register-startup.ps1")',
    '  Write-PostInstallLog "Starting supervisor bootstrap through SYSTEM scheduled task."',
    '  $schtasksPath = Join-Path $env:SystemRoot "System32\\schtasks.exe"',
    '  & $schtasksPath /Run /TN $watchdogTaskName *> $null',
    '  if ($LASTEXITCODE -ne 0) {',
    '    Write-PostInstallLog ("Scheduled task bootstrap failed with code " + $LASTEXITCODE + ". Falling back to direct bootstrap.")',
    '    & (Join-Path $installDir "start-supervisor.ps1")',
    '  }',
  ...
  const uninstallCleanupPs1 = [
    ...
    `try { Unregister-ScheduledTask -TaskName '${WATCHDOG_TASK_NAME.replace(/'/g, "''")}' -Confirm:$false -ErrorAction SilentlyContinue | Out-Null } catch {}`,
