...
function createPowerShellScripts() {
  ...
  const stopAgentPs1 = [
    ...
    '  $processName = [System.IO.Path]::GetFileNameWithoutExtension($ImageName)',
    '  Get-Process -Name $processName -ErrorAction SilentlyContinue | ForEach-Object { Stop-ProcessTreeById $_.Id }',
    '  taskkill /F /IM $ImageName /T *> $null',
    '}',
    'function Stop-ByLockFile {',
    '  if (-not (Test-Path $lockPath)) { return }',
    '  try {',
    '    $payload = Get-Content $lockPath -Raw | ConvertFrom-Json',
    '    if ($payload.pid) { Stop-ProcessTreeById ([int]$payload.pid) }',
    '  } catch {}',
    '}',
    'Write-StopLog "Stopping existing agent, launcher, and tunnel processes."',
    'Stop-ByLockFile',
    'Stop-ProcessImage $agentExeName',
    'Stop-ProcessImage $legacyAgentExeName',
    'Stop-ProcessImage "cloudflared.exe"',
    'Stop-ProcessImage "ngrok.exe"',
  ...
  const stopSupervisorPs1 = [
    ...
    '$processName = [System.IO.Path]::GetFileNameWithoutExtension($supervisorExeName)',
